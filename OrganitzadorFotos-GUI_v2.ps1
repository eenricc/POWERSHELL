    ...
    Foreach ($file in $files){      
        ...
        If ($resultat -eq "VIDEO"){
            ...
            Try{
                ...
                If (!(Test-Path -path "$desti\$any")) {New-Item "$desti\$any" -Type Directory}
                If (!(Test-Path -path "$desti\$any\$NomMes")) {New-Item "$desti\$any\$NomMes" -Type Directory}
                If (!(Test-Path -path "$desti\$any\$NomMes\$video")) {New-Item "$desti\$any\$NomMes\$video" -Type Directory}
                Copy-Item -path $file.fullname -destination $desti\$any\$NomMes\$video -Recurse -Force
                $Label3.Text = $file
                $Label3.Refresh()  
            }Catch{
        ...
        #ARXIS NO FILTRATS
        If (($resultat -ne "JPG") -and ($resultat -ne "VIDEO")){
